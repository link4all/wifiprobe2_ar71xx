...
//存储mac值 added by caosx
void get_maclist(u8 *addr,s8 signal){
	if(GLOBAL_AddrLocalNum >= ADDR_LOCAL_NUMBER){
		...
		for(index=0; index<GLOBAL_AddrLocalNum; index++){
			//if(memcmp(GLOBAL_AddrLocal[index],addr,MAC_ADDR_LEN)==0){
			if(memcmp(GLOBAL_AddrLocal[index],addr,6)==0){
				flag = 0;
				break;
			}
		}
		if(flag){
			memcpy(GLOBAL_AddrLocal[GLOBAL_AddrLocalNum],addr,6);
			memcpy(GLOBAL_AddrLocal[GLOBAL_AddrLocalNum]+6,&signal,1);
			GLOBAL_AddrLocalNum++;
		}
	}
}
					 ...
					 struct napi_struct *napi)
{
	...
	if (ieee80211_is_probe_req(fc) && GLOBAL_IsScan){ //added by caosx 20170902
		struct ieee80211_rx_status *sta = IEEE80211_SKB_RXCB(skb);
		get_maclist(hdr->addr2,sta->signal);
	}
